...
volatile boolean is_a_last=false;
...
void setup()
{
  ...
  delay(500);
  PCattachInterrupt(encoderPinA, doEncoderA, CHANGE);
  PCattachInterrupt(encoderPinB, doEncoderB, CHANGE);
}
...
void loop() {
  ...
  Serial.print(" ");
  Serial.println(env_count);
  if (i > 256) {
    //PCdetachInterrupt(encoderPinA);
    //PCdetachInterrupt(encoderPinB);
  }
}
